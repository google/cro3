...
def write_control_file(l_out_dir, l_suite, l_carrier, l_tag, l_tests):
    ...
    with open(os.path.join(l_out_dir, f"control.{primary_suite}-{l_carrier}-{l_tag}"),
              "w") as control_file:
        control_file.write(
                prefix.format(primary_suite=primary_suite, carrier=l_carrier, tag=l_tag, suites=suites) + l_tests + suffix)
...
for carrier in ['verizon', 'tmobile', 'att',
    ...
    if carrier in ['verizon','tmobile','att']:
        dut_check_suites.append('cellular-cq') # other carriers are sparsely deployed in the lab, thus CQ may fail to find DUTs
    ...
    write_control_file(out_dir, ['cellular_ota', 'cellular_repair'],
    ...
    write_control_file(out_dir, ['cellular_ota_flaky', 'cellular_repair'],
