...
def parse_raw_message(line, mm_raw_parser_level):
    ...
    line_parts = line.split("(translated)...", 1)
    parsed = ""
    useful_header_values = ""
    if len(line_parts) > 1:
        values = re.split("#012<<<<<<|#012>>>>>>", line_parts[1])
        pattern_found = False
        for value in values:
            if mm_raw_parser_level >= 3 and not pattern_found:
                if any(
                    value.strip().startswith(x)
                    for x in ("type ", "transaction ")
                ):
                    useful_header_values += value + "\n"
                if value.strip().startswith(
                    "Contents:"
                ):  # only useful for MBIM
                    pattern_found = True
                    parsed = "\n" + useful_header_values
            ...
            if mm_raw_parser_level >= 3:
                ...
                if any(
                    value.strip().startswith(x)
                    for x in ("length ", "value ", "tlv_length ")
                ):
                    continue
...
def parse_net_log(
    ...
    with open(file_name, encoding="ISO-8859-1") as ff:
        ...
        for line in ff:
            ...
            if (mm_raw_parser_level > 1 or hide_messages) and any(
                x in line for x in RAW_SUBSTRING_SEARCH
            ):
                continue

            if hide_messages and any(
                x in line
                for x in TRANSLATED_SUBSTRING_SEARCH + RAW_SUBSTRING_SEARCH
            ):
                continue
            ...
            if process:
                # Remove PID
                line = re.sub(
                    r"{0}\[[0-9]*\]".format(process), " " + process, line
                )

                # Remove logging level(i.e. DEBUG, INFO)
                line = re.sub(
                    "(DEBUG|INFO|WARNING|NOTICE)  {0}".format(process),
                    process,
                    line,
                )
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Hide `date-time` in all lines.",
    )
    parser.add_argument(
        "--hide-messages",
        "--hm",
    ...
    parser.add_argument(
        "-p", "--process", action="append", help="Filter lines by process name."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Add a timestamp that starts from 0"
    ...
    parser.add_argument(
        "--hide-signal-messages",
        "--hsm",
        action="store_true",
        default=False,
        help="Hide MM raw messages related to" "signal strength.",
    )
