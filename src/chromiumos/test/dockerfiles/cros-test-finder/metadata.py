...
def parse_local_arguments():
    ...
    parser.add_argument(
        "-path", dest="path", help="path (String): Path to write artifacts to."
    )
    parser.add_argument(
        "-force_path",
        dest="force_path",
    ...
    parser.add_argument(
        "-src", dest="src", help="path (String): The src tree path."
    )
...
class DockerPrep:
    ...
    def copy_services(self):
        ...
        shutil.copy(
            os.path.join(self.chroot_bin, "cros-test-finder"), self.full_out
        )
    ...
    def copy_dockerfiles(self):
        ...
        dockerfile_relative_dir = (
            "platform/dev/src/chromiumos/test/test_finder/docker"
        )
