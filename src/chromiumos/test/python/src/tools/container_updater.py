...
def _create_cleaned_image(temp_name: str, full_tag: str, tgts:list, sudo:bool):
...
def _respin_image(item: list, temp_name: str) -> str:
  ...
  sha, err, code = run(f'docker commit {temp_name} {temp_name}_localchangesfailed')
  if code != 0:
    print('commit failed')
    sys.exit(1)
  ...
  temp_name = str(int(time.time()))
  _, err, code = run(f'docker run -d --name {temp_name} {sha}'
                     f' sudo rm -r {item[2]}')
  if code == 0:
    return temp_name
  else:
...
class AutotestUpdater():
  ...
  def __init__(self, board: str, chroot_path: str, sysroot_path: str):
    ...
    _sysroot_full_autotest_path = os.path.join(chroot_path,
                                               sysroot_path,
                                               autotest_path)
  ...
  def prep(self) -> List[str]:
    ...
    if self.hasboard:
      ...
      if code != 0:
        ...
        raise Exception("Quickmerge failed on updating autotest artifacts.")
  ...
  def update_metadata(self) -> List[str]:
    ...
    pl = [os.path.join(self.chroot_path, 'tmp/autotest_metadata.pb'),
...
class ContainerUpdater():
  ...
  def _get_tag(self, service: str):
    ...
    firstTag = containers['containers'][self.board]['images'][service]['tags'][0]
    registryname = containers['containers'][self.board]['images'][service]['repository']['hostname']
    project = containers['containers'][self.board]['images'][service]['repository']['project']
  ...
  def _update_from_payloads(self):
    ...
    for service, payload in self.payloads.items():
        sha, tag = update_image(full_tag=self._get_tag(service),
                                tgts=payload,
  ...
  def _adjust_metadata(self, sha: str, tag: str, service:str):
    ...
    registryname = containers['containers'][self.board]['images'][service]['repository']['hostname']
    project = containers['containers'][self.board]['images'][service]['repository']['project']
    partial_tag = tag.replace(f'{os.path.join(registryname, project, service)}:', '')

    containers['containers'][self.board]['images'][service]['digest'] = sha
    containers['containers'][self.board]['images'][service]['tags'] = [partial_tag]
