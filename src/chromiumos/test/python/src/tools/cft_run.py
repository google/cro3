...
def parse_local_arguments() -> argparse.Namespace:
  ...
  parser.add_argument('-tags',
                      ...
                      nargs='*',
                      default=None,
                      help='tags(s) to run. Example usage: '
...
def download_metadata(args: argparse.Namespace) -> Tuple[str, str]:
  ...
  template = (f'gs://chromeos-image-archive/{args.board}-release/'
...
def validate_prereqs():
  ...
  if proc.returncode == 1 and 'invalid_grant: Bad Request' in stderr.decode():
    print('Docker not authd. Please run: \n'
...
def make_ssh_cmd(cmd: str) -> str:
  ...
  base = ('autossh -M 0 -o "ServerAliveInterval 5" -o "ServerAliveCountMax 2"'
...
class ProxyManager(object):
  ...
  def setup_dut_tunnel(self):
    ...
    self.tunnel_process = subprocess.Popen(cmd,
                                           shell=True,
                                           stdout=self.tunnel_file,
                                           stderr=self.tunnel_file)
  ...
  def setup_ports(self):
    ...
    if not self.wait_for_port(self.tunnel_port):
      print('DUT Tunnel not found!. Please ensure:\n'
  ...
  def startCacheServer(self):
    ...
    self.cache_process = subprocess.Popen(cmd,
                                          shell=True,
                                          stdout=self.cache_file,
                                          stderr=self.cache_file)
...
class CTRRunner(object):
  ...
  def download_ctr(self):
    ...
    run('wget https://chrome-infra-packages.appspot.com/dl/chromiumos/'
...
def main():
  ...
  with CTRRunner(args, md_path, image_path) as ctrRunner:
    ...
    if args.test:
      cmds.extend([ctrRunner.find_test,
                   ctrRunner.run_test])
    if args.publish:
      pass
    for cmd in cmds:
