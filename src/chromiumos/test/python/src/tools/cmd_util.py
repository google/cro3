...
"""Taring/cmd utilities to support container_util. NOTE forked from chromite"""
...
from src.tools import signals  # noqa: E402
...
class RunCommandError(CalledProcessError):
    ...
    def __init__(self, msg, result=None, exception=None):
        ...
        if result is None:
            ...
            raise TypeError(
                "result must be a CommandResult instance; got %r" % (result,)
            )
...
def _KillChildProcess(
    ...
    if not signals.RelaySignal(original_handler, signum, frame):
        ...
        raise TerminateRunCommandError(
            "Received signal %i" % signum, cmd_result
        )
...
def run(
    ...
    **kwargs,
    ...
    # Check the command.  This helps when RunCommand is deep in the call
    ...
    if isinstance(cmd, (str, bytes)):
        ...
        raise TypeError(
            "All command elements must be bytes/strings: %r" % (cmd,)
        )
