...
class BaseStation(object):
    ...
    def transmode(self):
        """Gets the TM of cell."""
        cmd = "CONFigure:LTE:SIGN:CONNection:{}:TRANsmission?".format(self._bts)
        return self._cmw.send_and_recv(cmd)
    ...
    def transmode(self, tm_mode):
        ...
        if not isinstance(tm_mode, TransmissionModes):
            raise ValueError(
                "tm_mode should be an instance of " "Transmission modes."
            )
    ...
    def uldl_configuration(self, uldl):
        ...
        if uldl not in range(0, 7):
            raise ValueError(
                "uldl configuration value should be between"
                " 0 and 6 inclusive."
            )
    ...
    def tdd_special_subframe(self, sframe):
        ...
        if sframe not in range(0, 10):
            raise ValueError(
                "tdd special subframe should be between 0 and 9" " inclusive."
            )
    ...
    def rb_configuration_dl(self, rb_config):
        ...
        if self.scheduling_mode == "RMC":
            ...
            cmd = "CONFigure:LTE:SIGN:CONNection:{}:RMC:DL {},{}," "{}".format(
                self._bts, rb, rb_pos, modulation
            )
            self._cmw.send_and_recv(cmd)
            ...
            if not isinstance(modulation, ModulationType):
                raise ValueError(
                    "Modulation should be of type " "ModulationType."
                )

            cmd = (
                "CONFigure:LTE:SIGN:CONNection:{}:UDCHannels:DL {},{},"
                "{},{}".format(self._bts, rb, start_rb, modulation.value, tbs)
            )
            self._cmw.send_and_recv(cmd)
    ...
    def rb_configuration_ul(self, rb_config):
        ...
        if self.scheduling_mode == "RMC":
            ...
            cmd = "CONFigure:LTE:SIGN:CONNection:{}:RMC:UL {},{}," "{}".format(
                self._bts, rb, rb_pos, modulation
            )
            self._cmw.send_and_recv(cmd)
    ...
    def validate_rb(self, rb):
        ...
        if bandwidth == LteBandwidth.BANDWIDTH_1MHz.value:
            if not 0 <= rb <= 6:
                raise ValueError(
                    "RB should be between 0 to 6 inclusive" " for 1.4Mhz."
                )
        elif bandwidth == LteBandwidth.BANDWIDTH_3MHz.value:
            if not 0 <= rb <= 10:
                raise ValueError(
                    "RB should be between 0 to 10 inclusive" " for 3 Mhz."
                )
        elif bandwidth == LteBandwidth.BANDWIDTH_5MHz.value:
            if not 0 <= rb <= 25:
                raise ValueError(
                    "RB should be between 0 to 25 inclusive" " for 5 Mhz."
                )
        elif bandwidth == LteBandwidth.BANDWIDTH_10MHz.value:
            if not 0 <= rb <= 50:
                raise ValueError(
                    "RB should be between 0 to 50 inclusive" " for 10 Mhz."
                )
        elif bandwidth == LteBandwidth.BANDWIDTH_15MHz.value:
            if not 0 <= rb <= 75:
                raise ValueError(
                    "RB should be between 0 to 75 inclusive" " for 15 Mhz."
                )
        elif bandwidth == LteBandwidth.BANDWIDTH_20MHz.value:
            if not 0 <= rb <= 100:
                raise ValueError(
                    "RB should be between 0 to 100 inclusive" " for 20 Mhz."
                )
    ...
    def reduced_pdcch(self):
        ...
        cmd = "CONFigure:LTE:SIGN:CONNection:{}:PDCCh:RPDCch?".format(self._bts)
        return self._cmw.send_and_recv(cmd)
    ...
    def tpc_power_control(self, set_type):
        ...
        if not isinstance(set_type, TpcPowerControl):
            raise ValueError(
                "set_type should be the instance of " "TpCPowerControl."
            )
...
class LteMeasurement(object):
    ...
    def stdev(self):
        ...
        """
        values = self._cmw.send_and_recv(
            "FETch:LTE:MEAS:MEValuation:MODulation:SDEV?"
        ).split(",")
        for i in range(0, 2):
            values[i] = _try_parse(values[i], int)
        for i in range(2, len(values)):
