...
from common import android_path
...
from common import stable_baseline
from common import stable_path
...
def createdb():
    ...
    # Create table
    c.execute(
        "CREATE TABLE commits (date integer, \
...
def update_stable(path, sha_list, origin):
    ...
    for commit in commits.splitlines():
        if commit != "":
            ...
            if found == []:
                c.execute(
                    "INSERT INTO stable(sha, origin) VALUES (?, ?)",
                    (
                        sha,
                        origin,
                    ),
...
def update_commits():
    ...
    for (
        commit
    ) in commits.splitlines():  # pylint: disable=too-many-nested-blocks
        if commit != "":
            ...
            ps = subprocess.Popen(
                ["git", "-C", chromeos_path, "show", sha],
                stdout=subprocess.PIPE,
            )
            spid = subprocess.check_output(
                ["git", "-C", chromeos_path, "patch-id"],
                stdin=ps.stdout,
                encoding="utf-8",
                errors="ignore",
            )
            ...
            if not chromium.match(subject):
                ...
                desc = subprocess.check_output(
                    ["git", "-C", chromeos_path, "show", "-s", sha],
                    encoding="utf-8",
                    errors="ignore",
                )
        ...
        if elem[0] == "+":
            ...
            if found:
                ...
                if found and found[0] == "drop" and found[1] == "upstream":
                    ...
                    c.execute(
                        "UPDATE commits SET reason=('') where sha='%s'" % sha
                    )
                    c.execute(
                        "UPDATE commits SET updated=('%d') where sha='%s'"
                        % (NOW(), sha)
