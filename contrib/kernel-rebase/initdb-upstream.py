...
def update_integrated_table(c, integrated):
    ...
    try:
        c.execute(
            "INSERT INTO tags(tag, timestamp) VALUES (?, ?)",
...
def handle(c, sha_range):
    ...
    for commit in commits.splitlines():
        if commit != "":
            ...
            ps = subprocess.Popen(
                ["git", "-C", upstream_path, "show", sha],
                stdout=subprocess.PIPE,
            )
            spid = subprocess.check_output(
                ["git", "-C", upstream_path, "patch-id"],
                stdin=ps.stdout,
                encoding="utf-8",
                errors="ignore",
            )
...
def update_patchids(c):
    ...
    for (sha,) in c.fetchall():
        ps = subprocess.Popen(
            ["git", "-C", upstream_path, "show", sha], stdout=subprocess.PIPE
        )
        spid = subprocess.check_output(
            ["git", "-C", upstream_path, "patch-id"],
            stdin=ps.stdout,
        ...
        c.execute(
            "UPDATE commits SET patchid=('%s') where sha='%s'" % (patchid, sha)
        )
    ...
    try:
        ...
        if sha:
            ...
            raise sqlite3.OperationalError(
                "Last handled SHA missing in database"
            )
