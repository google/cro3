...
file_handler = logging.FileHandler(f"log/bisect_branch-{datetime.now():%Y-%m-%d-%H_%M_%S}.log")
file_handler_format = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
file_handler.setFormatter(file_handler_format)

logging.basicConfig(
    level="NOTSET", format="%(message)s",
    datefmt="[%X]", handlers=[RichHandler(markup=True), file_handler]
)
...
parser.add_argument(
    ...
    '--triage',
    action='store_true',
    help=('triage mode, drop all conflicting patches which cannot be'
...
def is_patch_upstreamed(patches_upstream, patch):
    ...
    for entry in patches_upstream:
        if (patch['commit-title'] == entry['commit-title'] or
                patch['commit-title'].strip(FROMLIST_PREFIX)
                == entry['commit-title'] or
                patch['commit-title'].strip(FROMGIT_PREFIX)
                == entry['commit-title'] or
                patch['commit-title'].strip(BACKPORT_FROMGIT_PREFIX)
                == entry['commit-title'] or
                patch['commit-title'].strip(CHROMIUM_PREFIX)
                == entry['commit-title']):
            if args.debug:
...
def dispositions(patches_begin, patches_end, patches_upstream):
    ...
    for disp1 in dispositions_naive:
        for disp2 in dispositions_naive:
            ...
            if d1 == 'revert' and d2 == 'pick' and is_same(
                    patch1, patch2) and not is_fixup:
    ...
    for disp in dispositions_naive:
        ...
        if d == 'revert':
            ...
            for squash in to_squash:
                if (is_same(patch, squash['revert']) and
                        patch['content-hash'] and
...
def create_bisect_branch(begin,
                         end,
...
def verify_bisect_branch(begin, end, steps_cnt):
    ...
    shas = list_shas('kernel-upstream',
                     f'{KERNELUPSTREAM_BRANCH_PREFIX}{begin}..HEAD')
    shas_cnt = len(shas)
    log.info(f'There are {shas_cnt} commits on {bisect_branch} branch which '\
...
def alternative_apply_patches(sha, branch_prefix, patches):
    ...
    dropped = 0
    dropped_patches = []
    log.info((f'Checkout at {sha}: '
             f"{commit_subject('kernel-upstream', sha)}"))
    ...
    with patch_progress() as p:
        for commit in p.track(patches, description='Applying ChromeOS commits...'):
            if not cherry_pick_sha(commit['sha']):
                dropped_patches.append(commit)
                log.info(f'[b][red]DROP[/red][/b] {commit["sha"]} {commit["commit-title"]}')
            else:
                log.info(f'[b][green]PICK[/green][/b] {commit["sha"]} {commit["commit-title"]}')
...
def alternative_bisect_branch(begin, end):
    ...
    shas = list_shas('kernel-upstream',
                     f'{begin}..{KERNELUPSTREAM_BRANCH_PREFIX}{end}')
    shas.reverse()
    shas_count = len(shas)
    log.info(
    ...
    statistics_table = Table(
        "STEP", "FROM SHA", "TO SHA", "DROPPED", "BUILD", title="Bisection statistics")
    ...
    for step in range(steps_count):
        ...
        statistics_table.add_row(
            str(step+1), commit_first, commit_last, str(dropped), build_status_str)
        if build_status:
            log.info(f'Kernel at {commit_first} w/ ChromeOS patches builds properly')
            with sh.pushd('kernel-upstream'):
            ...
            if not args.always_pass_kernel:
                ...
                log.info('Correct build errors, resolve conflicts, upload fixes,')
