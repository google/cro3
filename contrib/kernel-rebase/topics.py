...
def update_sha(sha, topic, filename="", base=""):
    ...
    while filelist:
        ...
        if result:
            ...
            # be dropped.
            if disposition == "drop" and reason in (
                "stable",
                "android",
                "Intel",
            ):
                print(
                    "Disposition for '%s' is '%s' ('%s'), skipping"
        ...
        for (db_filename,) in c.fetchall():
            c.execute(
                "select sha from files where filename is '%s'" % (db_filename)
            )
            for (fsha,) in c.fetchall():
                ...
                if (
                    fsha != sha
                    and not db_filename.endswith("Makefile")
                    and not db_filename.endswith("Kconfig")
                ):
                    if (
                        base != ""
                        and not db_filename.startswith(base)
                        and get_topic(db_filename) != topic
                    ):
                        print(
                            "  Skipping '%s': base '%s' mismatch [%d-%d]"
...
def handle_topic(topic, subdir, name):
    ...
    if not c.fetchone():
        print(
            "Adding topic %d (%s), subdirectory/file '%s' to database"
            % (topic, name, subdir)
        )
        c.execute(
            "INSERT INTO topics(topic, name) VALUES (?, ?)",
            (
                topic,
                name,
            ),
    ...
    print(
        "Handling topic %d (%s), subdirectory/file '%s'" % (topic, name, subdir)
    )
    c.execute(
        "select sha, filename from files where filename like '%s%%'" % subdir
    )
    for (
        sha,
        filename,
