...
class CacheTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    ASSERT_EQ(CACHE_OP_SUCCESS,
              cache_map(&cache, TESTCACHEFILE, TESTLOCKNAME, 0x1000));
  }
...
TEST_F(CacheTest, InsertFromTwoProcesses) {
  ...
  if (fork() == 0) {
    ...
    for (int i = 0; i < child_insertions_cnt; i++)
      EXPECT_EQ(CACHE_INSERTION_SUCCESS, cache_insert(&cache2, "AAAAA"));
    cache_unmap(&cache2);
    exit(0);
  } else {
    for (int i = 0; i < parent_insertions_cnt; i++)
      EXPECT_EQ(CACHE_INSERTION_SUCCESS, cache_insert(&cache, "BBBBBB"));
  }
...
TEST_F(CacheTest, CacheInsertIfNotContains) {
  ...
  EXPECT_EQ(CACHE_CONTAINS_SUCCESS,
            cache_notcontains_insert(&cache, "AAAAAAA"));
  EXPECT_EQ(CACHE_INSERTION_SUCCESS,
            cache_notcontains_insert(&cache, "BBBBBBB"));
}
...
TEST_F(CacheTest, CacheHeaderSize) { EXPECT_EQ(sizeof(struct cachehdr), 16); }

TEST_F(CacheTest, InsertFailsLengthExceeded) {
  int insert_count = (0x1000 - sizeof(struct cachehdr)) / 4;

  for (int i = 0; i < insert_count; i++) cache_insert(&cache, "AAA");
...
TEST_F(CacheTest, InsertFailsLengthExceededAcrossProcesses) {
  ...
  if (fork() == 0) {
    struct cache cache2 = {0};
    cache_map(&cache2, TESTCACHEFILE, TESTLOCKNAME, 0x1000);
    for (int i = 0; i < insert_count / 2; i++) cache_insert(&cache2, "AAA");
    cache_unmap(&cache2);
    exit(0);
  }
...
TEST_F(CacheTest, MapInvalidCache) {
  EXPECT_EQ(CACHE_EINVAL,
            cache_map(NULL, TESTCACHEFILE, TESTCACHEFILE, 0x1000));
}
...
TEST_F(CacheTest, MapInvalidCacheSize) {
  struct cache cache2 = {0};
  EXPECT_EQ(CACHE_EINVAL, cache_map(&cache2, TESTCACHEFILE, TESTCACHEFILE, 0));
}
