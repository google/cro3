/* Imported from ${LINUX}/scripts/gcc-plugins */
...
static inline bool gimple_call_builtin_p(gimple stmt,
                                         enum built_in_function code) {
  tree fndecl;

  if (!is_gimple_call(stmt)) return false;
  fndecl = gimple_call_fndecl(stmt);
  if (!fndecl || DECL_BUILT_IN_CLASS(fndecl) != BUILT_IN_NORMAL) return false;
  return DECL_FUNCTION_CODE(fndecl) == code;
}

static inline bool is_simple_builtin(tree decl) {
  if (decl && DECL_BUILT_IN_CLASS(decl) != BUILT_IN_NORMAL) return false;
...
static inline bool gimple_clobber_p(gimple s __unused) { return false; }
...
static inline bool gimple_asm_clobbers_memory_p(const_gimple stmt) {
  ...
  for (i = 0; i < gimple_asm_nclobbers(stmt); i++) {
    ...
    if (!strcmp(TREE_STRING_POINTER(TREE_VALUE(op)), "memory")) return true;
  }
...
static inline bool cgraph_function_with_gimple_body_p(
...
static inline struct cgraph_node *cgraph_first_function_with_gimple_body(void) {
  ...
  for (node = cgraph_nodes; node; node = node->next)
    if (cgraph_function_with_gimple_body_p(node)) return node;
  return NULL;
}

static inline struct cgraph_node *cgraph_next_function_with_gimple_body(
    struct cgraph_node *node) {
...
static inline bool cgraph_for_node_and_aliases(
    cgraph_node_ptr node, bool (*callback)(cgraph_node_ptr, void *), void *data,
    bool include_overwritable) {
  cgraph_node_ptr alias;

  if (callback(node, data)) return true;
...
#define cgraph_create_edge_including_clones(                               \
    caller, callee, old_call_stmt, call_stmt, count, freq, nest, reason)   \
  cgraph_create_edge_including_clones((caller), (callee), (old_call_stmt), \
...
static inline bool tree_fits_shwi_p(const_tree t) {
  if (t == NULL_TREE || TREE_CODE(t) != INTEGER_CST) return false;

  if (TREE_INT_CST_HIGH(t) == 0 && (HOST_WIDE_INT)TREE_INT_CST_LOW(t) >= 0)
    return true;

  if (TREE_INT_CST_HIGH(t) == -1 && (HOST_WIDE_INT)TREE_INT_CST_LOW(t) < 0 &&
...
static inline gasm *as_a_gasm(gimple stmt) { return stmt; }

static inline const gasm *as_a_const_gasm(const_gimple stmt) { return stmt; }

static inline gassign *as_a_gassign(gimple stmt) { return stmt; }

static inline const gassign *as_a_const_gassign(const_gimple stmt) {
  return stmt;
}

static inline gcall *as_a_gcall(gimple stmt) { return stmt; }

static inline const gcall *as_a_const_gcall(const_gimple stmt) { return stmt; }

static inline gcond *as_a_gcond(gimple stmt) { return stmt; }

static inline const gcond *as_a_const_gcond(const_gimple stmt) { return stmt; }

static inline gdebug *as_a_gdebug(gimple stmt) { return stmt; }

static inline const gdebug *as_a_const_gdebug(const_gimple stmt) {
  return stmt;
}

static inline ggoto *as_a_ggoto(gimple stmt) { return stmt; }

static inline const ggoto *as_a_const_ggoto(const_gimple stmt) { return stmt; }

static inline gphi *as_a_gphi(gimple stmt) { return stmt; }

static inline const gphi *as_a_const_gphi(const_gimple stmt) { return stmt; }

static inline greturn *as_a_greturn(gimple stmt) { return stmt; }
...
static inline const char *get_decl_section_name(const_tree decl) {
  if (DECL_SECTION_NAME(decl) == NULL_TREE) return NULL;
...
static inline gasm *as_a_gasm(gimple stmt) { return as_a<gasm>(stmt); }
...
static inline gcall *as_a_gcall(gimple stmt) { return as_a<gcall>(stmt); }
...
static inline gphi *as_a_gphi(gimple stmt) { return as_a<gphi>(stmt); }
...
static inline cgraph_node_ptr cgraph_function_node(
    cgraph_node_ptr node, enum availability *availability) {
...
static inline ggoto *as_a_ggoto(gimple stmt) { return as_a<ggoto *>(stmt); }
...
#ifdef __cplusplus
static inline void debug_tree(const_tree t) { debug_tree(CONST_CAST_TREE(t)); }
