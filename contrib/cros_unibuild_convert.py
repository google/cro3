...
def prepend_all_lines(text, prepend):
    return "".join("{}{}\n".format(prepend, line) for line in text.splitlines())
...
Use of this source code is governed by a BSD-style license that can be
found in the LICENSE file.""".format(
...
def sh_getvar(script, varname):
    ...
    with tempfile.NamedTemporaryFile("w") as f:
        ...
        f.flush()
        res = subprocess.run(
            ["sh", f.name], stdout=subprocess.PIPE, check=True, encoding="utf-8"
        )
...
class CrosConfig:
    def __init__(self, public_yaml_raw, private_yaml_raw):
        with tempfile.NamedTemporaryFile(
            mode="w", delete=False
        ) as merged_tempfile, tempfile.NamedTemporaryFile(
            mode="w"
        ) as public_yaml_tempfile, tempfile.NamedTemporaryFile(
            mode="w"
        ) as private_yaml_tempfile:
            ...
            subprocess.run(
                [
                    "cros_config_schema",
                    "-o",
                    merged_tempfile.name,
                    "-m",
                    public_yaml_tempfile.name,
                    private_yaml_tempfile.name,
                ],
                check=True,
            )
    ...
    def run_host_command(self, *args):
        return subprocess.run(
            ["cros_config_host", "-c", self.merged_yaml] + list(args),
            check=True,
            encoding="utf-8",
...
class BoardOverlays:
    ...
    def __init__(self, board_name, checkout):
        ...
        self.public_overlay = (
            checkout / "src" / "overlays" / f"overlay-{board_name}"
        )
        ...
        for var, attr in self.MAKE_DEFAULTS_ATTRS:
            ...
            for script in (
                self.public_make_defaults,
                self.private_make_defaults,
            ):
        ...
        self.ec_extras_build_target = (
            sorted(list(self.ec_firmware_extras | self.pd_firmwares)) or None
        )
    ...
    def run_command(self, argv, *args, **kwargs):
        ...
        return subprocess.run(
            [
                ...
                self.ssh_identity,
                "-o",
                "UserKnownHostsFile={}".format(self.ssh_known_hosts_file),
                "-o",
...
genconf_schema = {
    ...
    "arc": {
        "build-properties": {
            "device": (
                M_PRIVATE,
                lambda d, _: d.arc_build_prop("ro.product.device"),
            ),
            ...
            "product": (
                M_PRIVATE,
                lambda d, _: d.arc_build_prop("ro.product.name"),
            ),
        },
    },
    "audio": {
    ...
    "firmware": {
        ...
        "bcs-overlay": (
            M_PRIVATE,
            lambda _, b: f"overlay-{b.board_name}-private",
        ),
...
def validate_gs_uri(uri):
    ...
    subprocess.run(
        ["gsutil", "stat", uri], check=True, stdout=subprocess.DEVNULL
    )
...
def parse_opts(argv):
    ...
    parser.add_argument(
        ...
        dest="duts",
        help=(
            "Hostname of DUT(s) to use for querying. "
            "Note: multiple duts can be passed, which is useful"
            " for converting a zerg board like hana or lars."
        ),
    )
    parser.add_argument(
        "--board", "-b", type=str, required=True, help="Board name to convert."
    )
    parser.add_argument(
...
def main(argv):
    ...
    for dut_string in opts.duts:
        dut_configs.extend(
            load_duts(dut_string, cros_checkout=opts.cros_checkout)
        )
    ...
    firmware_srcuris_path = (
        overlays.firmware_ebuild_path.parent / "files" / "srcuris"
    )
