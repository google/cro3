...
def _reset_hard_ref(reference):
    """Force reset to provided reference"""
    reset_cmd = ['git', 'reset', '-q', '--hard', reference]
    subprocess.run(reset_cmd, check=True)


def _reset_hard_head():
    """Force reset to local git head"""
    _reset_hard_ref('HEAD')


def _reset_hard_origin(branch):
    """Force reset to head of remote branch"""
    _reset_hard_ref('origin/%s' % branch)
...
class commitHandler:
    ...
    def __checkout_and_clean(self):
        ...
        checkout = ['git', 'checkout', '-q', self.branchname]
        ...
        _reset_hard_origin(self.current_branch)
    ...
    def __search_subject(self, sha):
        ...
        if self.branch not in self.commit_list:
            cmd = ['git', 'log', '--no-merges', '--format=%s',
    ...
    def __get_git_push_cmd(self, reviewers):
        ...
        """
        git_push_head = 'git push origin HEAD:refs/for/%s' % self.branchname
