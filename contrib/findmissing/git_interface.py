...
class commitHandler:
    ...
    def __init__(self, kernel, branch=None):
        ...
        current_branch_cmd = ['git', '-C', self.path, 'symbolic-ref', '-q', '--short', 'HEAD']
        self.current_branch = subprocess.check_output(current_branch_cmd, encoding='utf-8',
    ...
    def __reset_hard_ref(self, reference):
        ...
        reset_cmd = ['git', '-C', self.path, 'reset', '-q', '--hard', reference]
    ...
    def __checkout_and_clean(self):
        ...
        clean_untracked = ['git', '-C', self.path, 'clean', '-d', '-x', '-f', '-q']
        checkout = ['git', '-C', self.path, 'checkout', '-q', self.branchname]
    ...
    def __search_subject(self, sha):
        ...
        try:
            ...
            cmd = ['git', '-C', self.path, 'log', '--pretty=format:%s', '-n', '1', sha]
        ...
        if self.branch not in self.commit_list:
            cmd = ['git', '-C', self.path, 'log', '--no-merges', '--format=%s',
                   '%s..%s' % (self.merge_base, self.branchname)]
            subjects = subprocess.check_output(cmd, encoding='utf-8', errors='ignore')
    ...
    def __get_git_push_cmd(self, reviewers):
        ...
        """
        git_push_head = 'git -C %s push origin HEAD:refs/for/%s' % (self.path, self.branchname)
    ...
    def pull(self, branch=None):
        ...
        self.__setup()
        pull_cmd = ['git', '-C', self.path, 'pull', '-q']
    ...
    def cherry_pick_and_push(self, fixer_upstream_sha, fixer_changeid, fix_commit_message,
        ...
        try:
            self.status = 'changed'
            subprocess.run(['git', '-C', self.path, 'cherry-pick', '-n', fixer_upstream_sha],
                            check=True)
            subprocess.run(['git', '-C', self.path, 'commit', '-s', '-m', fix_commit_message],
                           check=True)
            ...
            subprocess.run(['git', '-C', self.path, 'commit', '--amend', '-m', commit_message],
                           check=True)
    ...
    def cherrypick_status(self, sha, branch=None, apply=True):
        ...
        try:
            ...
            result = subprocess.call(['git', '-C', self.path, 'cherry-pick', '-n', sha],
            ...
            diff = subprocess.check_output(['git', '-C', self.path, 'diff', 'HEAD'])
            if diff:
