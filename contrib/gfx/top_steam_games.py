...
def parse_script_stdout_json(script, args):
    ...
    try:
        cmd = [
            os.path.join(os.path.dirname(os.path.realpath(__file__)), script)
        ] + args
...
def main(args):
    ...
    parser.add_argument(
        "games_count", type=int, help="Number of games to print"
    )
    parser.add_argument(
    ...
    try:
        print(
            f"Preparing the list of the Top {opts.games_count} Linux games..."
        )
        cur_game_pos = 1
        cur_chart_page = 1
        ...
        games = []
        while (
            len(games) < opts.games_count and cur_chart_page <= MAX_CHART_PAGE
        ):
            game_ids = get_games_on_page(cur_chart_page)
            for game_id in game_ids:
                game_info = parse_script_stdout_json(
                    "steam_game_info.py", [game_id]
                )
        ...
        with open(opts.out_file, "w", encoding="utf-8") as out_file:
            if out_file_type == ".json":
                ...
                out_file.write(
                    "gameid,game_name,chart_pos,linux,windows,macos\n"
                )
                for game in games:
                    out_file.write(
                        ",".join(
