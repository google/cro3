...
cmd_data_sources = {
    "linux_version": {"cmd": "uname -a"},
    "packages": {
        "cmd": "dpkg-query -f '${binary:Package}##${source:Version}\n' -W '*'",
        "parser": package_list_parser,
    },
    "sources_list": {"cmd": "cat /etc/apt/sources.list"},
    "sources_list_d": {"cmd": "cat /etc/apt/sources.list.d/*"},
    "glx_device": {"cmd": "glxinfo -B | grep Device: | cut -f2- -d:"},
}
