...
def fetch_linux_kernel(kernel_path):
    ...
    try:
        subprocess.check_call(
            ["git", "fetch", os.getenv("LINUX"), "master"],
...
def cherry_pick(kernel_path, sha, bug_id):
    ...
    try:
        ...
        subprocess.check_call(
            ["git", "cherry-pick", "--abort"], cwd=kernel_path
        )
        return False

    subprocess.check_call(
        ["git", "commit", "--amend", "-s", "-m", fix_commit_message],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
        cwd=kernel_path,
    )
...
def modify_change_id(msg, kernel, kernel_path, test_change_ids):
    ...
    try:
        ...
        raise PatchApplierException(
            f"Failed to modify the change id for {kernel}"
        )
...
def apply_patch(sha, bug_id, kernel_versions, test_change_ids=None):
    ...
    if os.getenv("CHROMIUMOS_KERNEL") == "":
        raise PatchApplierException(
            "Environment variable CHROMIUMOS_KERNEL is not set"
        )
    ...
    for kernel in kernel_versions:
        ...
        if test_change_ids and cp_status[kernel]:
            ...
            modify_change_id(
                common.get_commit_message(kernel_path, new_patch_sha),
                kernel,
                kernel_path,
                test_change_ids,
            )
