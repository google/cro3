...
class TestContextGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        subprocess.check_call(
            ...
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
            cwd=self.stable_temp,
        )

        create_branch_and_commit_file(
            self.stable_temp, "linux-1.0.y", "file1", "random subject"
        )
        ...
        subprocess.check_call(
            ...
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
            cwd=self.stable_rc_temp,
        )

        create_branch_and_commit_file(
            self.stable_rc_temp, "v1.0", "file", "random subject"
        )
    ...
    def test_detect_relevant_commits(self):
        ...
        self.assertIn(
            "03e62fd67d3ab33f39573fc8787d89dc9b4d7255", cg.relevant_commits
        )
    ...
    def test_is_in_kernel(self):
        ...
        check = cg.is_in_kernel(
            self.kernel_temp1, TestContextGenerator.FIXES_SUBJECT, True
        )

        self.assertTrue(check)

        check = cg.is_in_kernel(
            self.kernel_temp2, TestContextGenerator.FIXES_SUBJECT, True
        )
