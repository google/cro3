...
class VersionRange:
    ...
    def from_str(value):
        ...
        # We are not going to worry about more than two constraints until we see it.
            ...
            raise VersionParseError(
                f'Version constraint has more than two parts: {parts}')
        # Handle the typical case of a single constraint operator.
            ...
            if dep == '=':
                ...
                if v_min[0] < 0:
                    raise VersionParseError(
                        f'The ~ constraint operator requires a major version: {value}')
                ...
                raise VersionParseError(f'Unrecognized operator: "{dep}"')
            else:
                major = max(v_min[0], 0)
                minor = max(v_min[1], 0)
                patch = max(v_min[2], 0)
    ...
    def to_ebuild_str(self, category_and_name):
        ...
        if self.max_inclusive:
            ...
            suffix = '_alpha'
        if not self.max:
            return f'{min_bound}{category_and_name}-{self.min[0]}.{self.min[1]}.{self.min[2]}:='
        ...
        for x in range(len(self.min)):
            ...
            return f'~{category_and_name}-{self.min[0]}.{self.min[1]}.{self.min[2]}:='

        if self.min_inclusive and not self.max_inclusive and self.min[x] + 1 == self.max[x]:
            if x == 0 and self.min[1] == 0 and self.min[2] == 0:
                return f'={category_and_name}-{self.min[0]}*:='
            if x == 1 and self.min[2] == 0:
                return f'={category_and_name}-{self.min[0]}.{self.min[1]}*:='
        ...
        # binary packages.
        return f'{min_bound}{category_and_name}-{self.min[0]}.{self.min[1]}.{self.min[2]} {max_bound}{category_and_name}-{self.max[0]}.{self.max[1]}.{self.max[2]}{suffix}:='
...
class DepGraphNode:
    ...
    def __repr__(self):
        dep_list = [(a['name'], a['req']) for a in self.dependencies]
        return f'{dep_list}'
            ...
            for sys_version, sys_features in self.system_crates[dep_name].items():
                ...
                if not sys_crate_is_empty and req.contains(sys_version):
                    if not args.no_substitute or (not is_required and dep_node is None):
                        found = True
                        break
                if VersionRange.from_str(f'^{sys_version}').contains(dep_version):
                    if sys_crate_is_empty:
...
def get_crate_path(package, staging_dir):
    ...
    return os.path.join(
        staging_dir, 'crates', f'{package["name"]}-{package["version"]}.crate')


def get_clean_crate_name(package):
    """Clean up crate name to {name}-{major}.{minor}.{patch}."""
    return f'{package["name"]}-{get_clean_package_version(package["version"])}.crate'
...
def get_ebuild_path(name, version, staging_dir, make_dir=False):
    ...
    ebuild_path = os.path.join(
        ebuild_dir,
        f'{name}-{get_clean_package_version(version)}.ebuild')
        ...
        try:
            ...
            bounds = (
                VersionRange.from_str(dep['req'])
                            .to_ebuild_str(f'dev-rust/{dep["name"]}')
            )
            deps.append(f'\t{bounds}')
            ...
            deps.append(f"\t$(die 'Please replace with proper DEPEND: {dep['name']} = {dep['req']}')")
...
def package_ebuild(package, ebuild_dir, crate_dependencies, filter_target):
    ...
    if package['version'] != get_clean_package_version(package['version']):
        autogen_notice = '\n'.join([
            autogen_notice,
            f'# ${{PV}} was changed from the original {package["version"]}'
        ])
...
def upload_gsutil(package, staging_dir, no_upload=False):
    ...
    ret = subprocess.run([
        ...
        f'gs://chromeos-localmirror/distfiles/{crate_name}'
    ]).returncode
...
def process_empty_package(name, version, features, args):
    ...
    if features:
        ...
        crate_features = f'CROS_RUST_EMPTY_CRATE_FEATURES=( {features_list} )'
...
def main(argv):
    ...
    if not args.dry_run:
        for p in metadata['packages']:
            if p['name'] in args.skip:
                continue
            # Ignore packages that are part of the local workspace (i.e. not third party)
            if args.third_party_only and p['source'] is None:
                continue
    ...
    print(f'Generated ebuilds can be found in: {display_dir}')
...
def parse_args(argv):
    ...
    parser.add_argument('--third-party-only',
                        action='store_true',
                        help=(
                            'Only generate ebuilds for third party dependencies (i.e. crates ' +
                            'that are not part of the local workspace).'
                        )
    )
