...
class PullRequest:
    ...
    def _parse_pull_request(self):
        ...
        m = re.findall(r'The following changes since commit ([a-f0-9]+):',
                     self._pull_request)
        if not m:
        ...
        m = re.findall(r'available in the Git repository at:\s+(\S+)\s(\S+)\n',
                       self._pull_request, re.MULTILINE)
        if not m:
        ...
        m = re.findall(r'for you to fetch changes up to ([a-f0-9]+):',
                       self._pull_request)
        if not m:
